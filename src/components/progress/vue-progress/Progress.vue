...
export default {
  ...
  methods: {
    init() {
      ...
      if (this.svgStyle) { tmp.svgStyle = this.svgStyle; }
      if (this.text) { tmp.text = this.text; }
      if (this.fill) { tmp.fill = this.fill; }
      if (this.from) { tmp.from = this.from; }
      if (this.to) { tmp.to = this.to; }
      if (this.step) { tmp.step = this.step; }

      let options = extend(tmp, this.options || {});
      ...
      switch (this.type) {
        ...
        case PATH: {
          ...
          if (paths.length === 0) {
            throw new Error('[VueProgress Error] Path not found in slot svg.');
          }
